...
class InterAtomsTranslator:
    ...
    def _match_plane_with_group(
        ...
        for atom in inter_atoms_plane_coordinates.points:
            ...
            for plane_i, plane in enumerate(planes):
                min_distances: np.ndarray = DistanceMeasurer.calculate_min_distances(np.array([atom]), plane.points)
    ...
    def _copy_inter_atoms_points_to_rest_planes(
        ...
        min_allowed_dist: np.floating | float = atom_params.MIN_ALLOWED_DIST_BETWEEN_ATOMS
        ...
        for plane_i, plane in enumerate(planes):
            if plane_i in plane_group_map:
                ...
                min_dist: np.floating = np.min(
                    DistanceMeasurer.calculate_min_distances(
            ...
            if to_try_to_reflect_inter_atoms:
                ...
                rotated_min_dists: np.ndarray = DistanceMeasurer.calculate_min_distances(
