...
description: Build, rebuild, and validate ordered, repeating intercalated layers at carbon polygon centers, vertices, and edge midpoints, producing one_ch-polygon-{type}-v{i}-Claude.csv files.
---
...
10. Write only through `write_final_structure`, with `author="Claude"`,
    ...
    `one_ch-polygon-{type}-v{i}-Claude.csv`, where `i` starts at 1 independently for each polygon
