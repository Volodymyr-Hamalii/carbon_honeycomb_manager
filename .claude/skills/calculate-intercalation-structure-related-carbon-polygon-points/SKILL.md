...
description: Build, rebuild, and validate intercalated structures using carbon polygon centers, vertices, and edge midpoints, producing final_one_ch-v{i}[-{stacking}]-Claude.csv files.
---
...
9. Write only through `write_final_structure`, with `author="Claude"`. The result must be
   `final_one_ch-v{i}[-{stacking}]-Claude.csv`; never create an intermediate coordinate file and
